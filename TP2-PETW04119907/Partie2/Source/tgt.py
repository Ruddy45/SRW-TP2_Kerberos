...
dataUser = u""
messageBadArg = u"L'argument doit être de la forme : IDc-ADc-IDv-TICKETtgs."
...
# On compare les données du ticket TGS en le déchiffrant
ticket = function.CaesarCipher(function.VerifyCell(bdd, nameTgs)[1], ticket, -1)
dataUser = function.Split(ticket, letterToSplit)

# Si les données de l'utilisateur ne correspondent pas au ticket TGS
# Le nom du client, son adresse ou encore si le ticket est expiré
if nameClient != dataUser[0] or ipClient != dataUser[1] or nameTgs != dataUser[2] or (int(dataUser[3]) + int(dataUser[4])) < int(time.time()):
    print(u"Non, le ticket n'est pas valide.")
    sys.exit()

print(u"Donnée du Ticket TGS : " + str(ticket))
print(u"Ticket V : " + str(function.CaesarCipher(serveur[1], str(nameClient) + str(letterToSplit) + str(ipClient) + str(letterToSplit) + str(nameServer) + str(letterToSplit) + str(int(time.time())) + str(letterToSplit) + str(durationTicket), 1)))
